...
namespace OrnekProje.Areas.Customer.Controllers
{
    [Area("Customer")]
    ...
    public class CartController : Controller
    {
        ...
        public IActionResult Summary(ShoppingCartVM model)
        {
            ...
            foreach (var item in ShoppingCartVM.ListCart)
            {
                ...
                _db.OrderDetails.Add(orderDetails); //siparişleri veri tabanına kaydetme işlemi yapmış olduk.
            }
        ...
        private Payment PaymentProcess(ShoppingCartVM model) //aşaığıda githubdan alınan kod(lyzico)
        {
            Options options = new Options();
            options.ApiKey = "sandbox-DHv3zgNs4mS8iItRrLJ3nHES7lDCnurG"; //API anahtarı
            options.SecretKey = "sandbox-px5fsfYQrN5XjV2Cveyx3nsiMYv8sJKd"; //güvenlik anahtarı
        ...
        public IActionResult Decrease(int cartId)
        {
            ...
            if (cart.Count==1)
            {
                var count = _db.ShoppingCart.Where(u => u.ApplicationUser == cart.ApplicationUser).ToList().Count();
